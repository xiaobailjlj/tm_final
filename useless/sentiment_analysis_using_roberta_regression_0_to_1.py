...
def preprocess_function(examples):
    # Clean text
    text = examples["sentence_text"].lower()
    text = re.sub(r'[^\w\s]', '', text)

    # Normalize bias score to [0,1]
    label = (examples["bias_score"] - 0) / (3 - 0)
...
if __name__ == '__main__':
    ...
    print(f'ds["train"][0]: {ds["train"][1]}')
    print(f'ds["train"][0]: {ds["train"][2]}')
    print(f'ds["train"][0]: {ds["train"][99]}')
