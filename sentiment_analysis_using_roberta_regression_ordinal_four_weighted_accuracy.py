...
def compute_metrics(eval_pred):
    ...
    # class_distribution = dict(zip(unique_labels, label_counts / len(labels)))
    ...
    #         per_class_accuracy[f"class_{int(class_label)}_accuracy"] = float(class_acc)

    # Ordinal metrics
    abs_diff = np.abs(predictions - labels)
    # ordinal_metrics = {
    #     "exact_match": float(np.mean(abs_diff == 0)),  # Perfect predictions
    #     "off_by_one": float(np.mean(abs_diff == 1)),  # Off by 1 class
    #     "off_by_two": float(np.mean(abs_diff == 2)),  # Off by 2 classes
    #     "off_by_three": float(np.mean(abs_diff == 3)),  # Off by 3 classes
    #     "mean_abs_error": float(np.mean(abs_diff))  # Average distance from true class
    # }
    ...
    #         if mask.any():
    #             count = np.sum((predictions == pred_label) & mask)
    #             confusion[f"true_{int(true_label)}_pred_{int(pred_label)}"] = int(count)

    # Calculate stricter ordinal accuracy metrics
    # For balanced evaluation, weight by inverse class frequency
    ...
    ordinal_penalties = abs_diff ** 2  # Square the difference to penalize larger mistakes more
    ...
    metrics = {
        ...
        "weighted_accuracy": weighted_accuracy_distance,   # weighted_accuracy   weighted_accuracy_distance
    ...
    # # Log class distribution and confusion matrix for analysis
    # print("\nClass Distribution:")
    # for label, freq in class_distribution.items():
    #     print(f"Class {int(label)}: {freq:.3f}")
    #
    # print("\nConfusion Matrix:")
    ...
    #     row = [f"{true_label}"]
    #     for pred_label in unique_labels:
    #         key = f"true_{int(true_label)}_pred_{int(pred_label)}"
    #         row.append(str(confusion.get(key, 0)))
    #     print("\t".join(row))
