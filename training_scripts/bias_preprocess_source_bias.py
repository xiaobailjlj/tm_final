...
columns_to_keep = ["id_event", "id_article", "article_bias", "source_bias"]
...
for _, row in data.iterrows():
    for id_sentence, text_col in zip(sentence_ids, text_columns):
        ...
        if pd.notnull(sentence_text):
            ...
            if row["source_bias"] == "left":
                bias_score = 0
            elif row["source_bias"] == "left-center":
                bias_score = 1
            elif row["source_bias"] == "least":
                bias_score = 2
            elif row["source_bias"] == "right-center":
                bias_score = 3
            elif row["source_bias"] == "right":
                bias_score = 4
            elif row["source_bias"] == "right-extream":
                bias_score = 4
            else:
                bias_score = 5
...
# Save to CSV
processed_data.to_csv("news_bias_dataset/preprocessed_dataset_source_bias.csv", index=False)
