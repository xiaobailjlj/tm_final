...
def train_on_fake_labels(trainer, ds, raw_train_ds, raw_test_ds):
    new_data = pd.read_csv('./news_bias_dataset/augmented_dataset.csv', delimiter=',')
    new_ds = Dataset.from_pandas(new_data)
    new_ds = new_ds.map(preprocess_function, remove_columns=["sentence_text"])
    ...
    combined_train_ds = combined_train_ds.map(preprocess_function, remove_columns=["sentence_text", "bias_score"])
    ...
    ds["test"] = ds["test"].map(preprocess_function, remove_columns=["sentence_text", "bias_score"])
